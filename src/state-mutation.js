...
export const makeUpdateReverseRelationship = (
  ...
  return (foreignResources) => {
    const idx = foreignResources.findIndex(
    ...
    const relCase = [singular, plural].find((r) =>
    ...
    if (Array.isArray(foreignResourceRel) && !newRelation) {
      const relIdx = foreignResourceRel.findIndex(
...
const stateContainsResource = (state, resource) => {
  ...
  if (hasOwnProperties(state, updatePath)) {
    return (
      state[resource.type].data.findIndex((item) => item.id === resource.id) >
      -1
    );
  }
...
export const updateRelationship = (
  ...
  newState = imm.set(
    newState,
    updatePath.concat(...[idx, "relationships", relationshipName]),
    relationship
  );
  ...
  if (stateContainsResource(state, resource)) {
    ...
    if (resources[idx].hasOwnProperty("relationships")) {
      Object.keys(resources[idx].relationships).forEach((relationship) => {
        if (
          !hasOwnProperties(resource, ["relationships", relationship, "data"])
        ) {
...
export const removeResourceFromState = (state, resource) => {
  const index = state[resource.type].data.findIndex(
    (e) => e.id === resource.id
  );
...
export const ensureRelationshipInState = (
  state,
  { type, id },
  relationship
) => {
  ...
  return hasOwnProperties(state, path)
    ? state
    : imm.wrap(state).set(path, null).value();
};
