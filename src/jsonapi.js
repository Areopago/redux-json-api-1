...
const {
  ...
  API_DELETED,
  API_DELETE_FAILED,
  API_RELATIONSHIP_WILL_UPDATE,
  API_RELATIONSHIP_UPDATED,
  API_RELATIONSHIP_UPDATE_FAILED,
  API_RELATIONSHIP_WILL_DELETE,
  API_RELATIONSHIP_DELETED,
  API_RELATIONSHIP_DELETE_FAILED,
} = actionTypes;
...
const apiRelationshipUpdateFailed = createAction(
  API_RELATIONSHIP_UPDATE_FAILED
);
...
const apiRelationshipDeleteFailed = createAction(
  API_RELATIONSHIP_DELETE_FAILED
);
  ...
  return (dispatch, getState) => {
    ...
    return new Promise((resolve, reject) => {
      apiRequest(endpoint, axiosConfig)
        .then((json) => {
          dispatch(apiRead({ finalEndpoint, dominio, options, ...json }));
...
const getRelationshipEndpoint = (resource, relationship, axiosConfig) => {
  if (
    hasOwnProperties(resource, ["relationships", relationship, "links", "self"])
  ) {
...
export const readRelationship = (resource, relationship, queryString = "") => {
  return (dispatch, getState) => {
    const { axiosConfig } = getState().api.endpoint;
    const endpoint =
      getRelationshipEndpoint(resource, relationship, axiosConfig) +
      queryString;
...
export const replaceRelationship = (resource, relationship, data) => {
  return (dispatch, getState) => {
    ...
    const { axiosConfig } = getState().api.endpoint;
    const endpoint = getRelationshipEndpoint(
      resource,
      relationship,
      axiosConfig
    );
...
export const reducer = handleActions(
  {
    [API_SET_AXIOS_CONFIG]: (state, { payload: axiosConfig }) => {
      return imm
        .wrap(state)
        .set(["endpoint", "axiosConfig"], axiosConfig)
        .value();
    },
    ...
    [API_READ]: (state, { payload }) => {
      const resources = (Array.isArray(payload.data)
        ? payload.data
        : [payload.data]
      )
      ...
      const finalState = addLinksToState(
        newState,
        payload.links,
        payload.options
      );
    ...
    [API_RELATIONSHIP_READ]: (
      state,
      { payload: { resource, relationship, data } }
    ) => {
    ...
    [API_WILL_UPDATE]: (state, { payload: resource }) => {
      ...
      return setIsInvalidatingForExistingResource(
        newState,
        { type, id },
        IS_UPDATING
      )
    ...
    [API_RELATIONSHIP_WILL_UPDATE]: (
      ...
      const newState = ensureRelationshipInState(
        state,
        {
          type,
          id,
...
// Selectors
export const getResourceTree = (state, resourceType) =>
  state.api[resourceType] || { data: [] };
export const getResourceData = (state, resourceType) =>
  getResourceTree(state, resourceType)?.data || [];
...
const getIdList = (identifiers, idList) =>
  idList || identifiers.map((identifier) => identifier.id);
...
export const getResources = createCachedSelector(
  ...
  (_state, identifiers, idList) =>
    getIdList(identifiers, idList).map((id) => `${id}`),
  (resources, idList) =>
    resources.filter((resource) => idList.includes(`${resource.id}`))
)((_state, identifiers, idList) => {
...
export const getRelatedResources = (state, identifier, relationship) => {
  ...
  return Array.isArray(relationshipData)
    ? getResources(state, relationshipData)
    : getResource(state, relationshipData);
};
